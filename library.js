let myCalculator = {
    init: function (target) {
        target.innerHTML = '<h1>Calculator Design Using HTML Layout</h1>\n' +
            ...
            '        <input type="button" name="" value="/" class="global share">\n' +
            '    </div>\n' +
            '    <div class="third-row">\n' +
            ...
            '        <input type="button" name="" value="X" class="global multiply">\n' +
            '    </div>\n' +
            ...
            '        <input type="button" name="" value="-" class="global minus">\n' +
            '    </div>\n' +
            '    <div class="conflict">\n' +
            '        <div class="left">\n' +
        ...
        butEqual.addEventListener('click', function () {
            ...
            else if (myCalculator.previousAction === 'minus') {
                myCalculator.result.value = parseInt(myCalculator.previousValue) - parseInt(myCalculator.result.value);
            }
            else if (myCalculator.previousAction === 'share') {
                myCalculator.result.value = parseInt(myCalculator.previousValue) / parseInt(myCalculator.result.value); 
            } 
        ...
        butMinus.addEventListener('click', function() {
            myCalculator.previousValue = myCalculator.result.value;
            myCalculator.previousAction = 'minus';
            myCalculator.result.value = 0;
        })

        let butShare = document.querySelector('.share');
        butShare.addEventListener('click', function() {
            myCalculator.previousValue = myCalculator.result.value;
            myCalculator.previousAction = 'share';
            myCalculator.result.value = 0;
        })
        ...
        butMultiply.addEventListener('click', function() {
            myCalculator.previousValue = myCalculator.result.value;
            myCalculator.previousAction = 'multiply';
            myCalculator.result.value = 0;
        })
